...
public class Max extends AggregationFunction {
        ...
        for (int i = 0; i < data.size(); i++) {
            for (int j = 0; j < data.get(i).get(groupByField).size(); j++) {
                if (!temp.containsKey(data.get(i).get(groupByField).get(j))) {
                    temp.put(data.get(i).get(groupByField).get(j), Double.parseDouble(data.get(i).get("max_" + aggregationField).get(j)));
                } else {
                    double value = Double.parseDouble(data.get(i).get("max_" + aggregationField).get(j));
    ...
    private HashMap<String, ArrayList<String>> formResult(HashMap<String, Double> data) {
        ...
        result.put("max_" + aggregationField, new ArrayList<>());
        result.put(groupByField, new ArrayList<>());

        for (Map.Entry<String, Double> pair : data.entrySet()) {
            result.get("max_" + aggregationField).add("" + pair.getValue());
